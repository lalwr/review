...
- 테스트하기 쉬운 코드로 개발하기
  ...
  - 방법3 : 두 부류 코드가 만나는 가장자리는 어떻게 테스트 해야 하나? 
    ...
    - 상태 검증 - 문제 극복 방안
      ...
      - 난해한 코드가 아니다
      - 구현된 코드를 사용하지 않고 굳이 어려운 길을 택할 이유거 앖다
...
- 끝으로
  ...
  - 위 내용과 관련된 제 블로그 참조
    - http://jwchung.githib.io/testing-oh-my
...
- 방법에 대한 이야기
  - 테스트
    - 1-1 기존 코드에 테스트 추가하기
      ...
      - 테스트코드는 수정하지 않고 리팩토링을 진행한다.(리팩토링은 테스트가 꺠지지 않는 범위 내에서만)
